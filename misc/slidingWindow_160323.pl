...
our ($round,$random,$txt,$txtg,$cutoff,$wig,$infile, $csv, $step, $h, $outdir,$size,$fasta, $log, $ref_genome,$tan,$indir,$inc,$sortby,$weight_ceiling,$noweight,$custom,$wc);
GetOptions(
'wig:s' => \$wig,
'ref:s' => \$ref_genome,
'cutoff:i'=>\$cutoff,
...
for (my $i=0;$i<scalar @allWindows;$i++){
    ...
    my @c = $seq =~ /$ta/g;
    my $TAsites = scalar @c;
    #If there are no TA sites then shouldn't perform division calcs or else error
    if ($TAsites==0){
    	$avgInsert=0;
    	$pval=1;
    	}
    else{
    	$avgInsert=sprintf("$round",$insertions/$TAsites);
    	$pval=pvalue($avgInsert,$TAsites);
    }
